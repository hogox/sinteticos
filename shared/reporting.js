export function composeStepReason(persona, task, action, screen, certainty) {
    const behavior = persona.digital_level === "high"
...
export function buildFindings(task, persona, status, rng, coverageData = {}) {
    ...
    const { totalCandidates, totalConnected, coverageRatio, fallbackSteps, retriedSteps, retriedSuccessfully = 0, totalRetryAttempts = 0, stepLog = [], screenTransitions = [] } = coverageData;
    ...
    if (stepLog.length > 2) {
        const screens = stepLog.map((s) => s.screen).filter((v) => Boolean(v));
        const screenCounts = screens.reduce((acc, s) => {
            acc[s] = (acc[s] || 0) + 1;
            return acc;
        }, {});
