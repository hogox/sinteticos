...
interface ChatRun {
  ...
  step_log?: Array<{ action: string; screen: string; [k: string]: unknown }>;
  report_summary?: string;
  report_details?: {
    prioritized_findings?: Array<{ label: string; [k: string]: unknown }>;
  };
}
...
function compactJoin(items: unknown[]): string {
...
function descriptionFragment(value: unknown): string {
...
function getAnchoredRun(runs: ChatRun[] | undefined, anchorRunId: string | null): ChatRun | null {
  if (!anchorRunId) return null;
  return (runs || []).find((run) => run.id === anchorRunId) || null;
}

function hasEvidenceSignal(message: string): boolean {
  const normalized = asText(message).toLowerCase();
  return ["run", "corrida", "naveg", "click", "paso", "pantalla", "frustr", "aband", "hice", "vi"].some((token) =>
    normalized.includes(token)
  );
}

function asksOutsideContext(message: string): boolean {
  const normalized = asText(message).toLowerCase();
...
function summarizeRun(run: ChatRun | null, tasks: ChatTask[] | undefined): string {
  if (!run) return "";
...
function profileLine(persona: ChatPersona): string {
...
function followUpQuestion(message: string): string {
  const normalized = asText(message).toLowerCase();
...
function conversationalReply(persona: ChatPersona, message: string): string {
  const normalized = asText(message).toLowerCase();
...
function inferLocalHypothesisVerdict(persona: ChatPersona, message: string): HypothesisVerdict {
  const normalized = asText(message).toLowerCase();
...
export function buildLocalPersonaReply({
  persona,
  tasks = [],
  runs = [],
  message = "",
...
export function buildPersonaChatContext({
  persona,
  project,
  tasks = [],
  runs = [],
